...
{	
	...
	while (cursize < filesize) {
		...
		int readcnt;
		readcnt = fread(snd_pkt.data, 1, sizeof(snd_pkt.data), fd);
		...
		while (1) {
			...
			int numbytes;
			if ((numbytes = sendto(sockfd, &snd_pkt, sizeof(snd_pkt.header) + readcnt, 0,(struct sockaddr *)&client_info, len)) == -1) 
			{
				printf("sendto error\n");
				return 0;
			}
			...
			while (!got_rcv && ((clock()*1000)/CLOCKS_PER_SEC - t) < TIMEOUT);
